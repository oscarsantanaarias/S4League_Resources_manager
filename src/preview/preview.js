...
// A texture name inside a .scn sits in a fixed 1024 byte slot, NUL padded, so a
// replacement can be written in place as long as it fits. This was defined in
// main.js and server.js but used here, where neither is in scope: patching a
// texture threw "SCN_TEXTURE_SLOT_SIZE is not defined" every time.
...
function getScnTextureRefs(buffer){
  ...
  for(const item of getAsciiStrings(buffer, 4)){
    ...
    if(found.has(key)){
      ...
      hit.count++;
      hit.offsets.push(item.offset);   // a .scn names the same texture once per node
      continue;
    }
...
function patchScnTextureName(buffer, oldTexture, newTexture){
  ...
  for(const ref of getScnTextureRefs(buffer)){
    ...
    // every node that names this texture has its own slot, so patch them all.
    // Only rewriting the first one left the rest pointing at the old file.
    for(const offset of (ref.offsets || [ref.offset])){
      newBytes.copy(output, offset);
      output.fill(0, offset + newBytes.length, Math.min(output.length, offset + SCN_TEXTURE_SLOT_SIZE));
      changed++;
    }
  }
