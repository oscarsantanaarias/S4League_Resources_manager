...
class FridaProcessController(ProcessController):
    ...
    def flush_write_watch(self) -> int:
        count: int = self._frida_rpc.flush_write_watch()
        return count

    def get_stolen_writes(self) -> List[Dict[str, Any]]:
        """
        Everything the packer rewrote over already-initialized memory, as
        {address, before, after}. Recorded by the agent while the process ran.
        """
        value: List[Dict[str, Any]] = self._frida_rpc.get_stolen_writes()
        return value
