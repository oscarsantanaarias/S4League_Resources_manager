...
async function convertXmlFileToXbn(inputPath, outputPath, options = {}) {
...
async function convertXbnFileToXml(inputPath, outputPath, options = {}) {
  const buffer = await fs.readFile(inputPath);
  const offset = { pos: 0 };

  const header = buffer.readInt32LE(offset.pos);
  offset.pos += 4;
  const version = buffer.readFloatLE(offset.pos);
  offset.pos += 4;
