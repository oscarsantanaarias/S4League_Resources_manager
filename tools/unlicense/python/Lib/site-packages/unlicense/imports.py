...
def _is_indirect_jmp(code_section_data: bytes, offset: int) -> bool:
    """
    Check if the instruction at `offset` is an `FF25` jmp.

    `_is_wrapped_thunk_jmp` only accepts `FF25` when the following byte is
    `8B` or `C0`, which misses plain MSVC thunk tables (runs of back-to-back
    `jmp dword ptr [IAT]`, where the byte at +6 is another `FF`). Those thunks
    keep reading the original IAT, which still holds packer wrapper addresses
    after the dump, so the dumped binary crashes on the first CRT call.
    Destinations are validated further down the same way `FF15` calls are.
    """
