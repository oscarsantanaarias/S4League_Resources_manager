...
const RF_TRANSPARENT = 2, RF_CUTOUT = 4, RF_NO_CULLING = 8, RF_FLARE = 32, RF_NO_DEPTH_WRITE = 64;
...
function makeMaterial(tex, flag, name = '') {
  ...
  const transparent = cutout || blend;
...
export async function buildScene(scn, getTexture, pose = POSE_BIND) {
  ...
  if (animated.length) {
    ...
    animate = (tick, clip) => {
      ...
      const wOf = hasSceneTRS ? conSocket(nodeResolver(scn, pz)) : worldOf;
      for (const it of animated) {
        const local = it.morphAnim ? morphLocal(it, tick) : it.base;
        const wp = worldPositions(scn, { ...it.m, positions: local }, pz, wOf);
