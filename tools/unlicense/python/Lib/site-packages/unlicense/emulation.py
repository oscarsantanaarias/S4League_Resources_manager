...
# Themida steals an API's first instructions, copies them into the wrapper, and
# jumps a few bytes *into* the API. Emulation then lands on `API+N`, which is not
# an export address, so a plain dict lookup never fires and the wrapper is
# reported as unresolvable. Map such an address back to the export it belongs to.
STOLEN_PROLOGUE_MAX = 0  # EXPERIMENTO
_SORTED_EXPORTS: Dict[int, List[int]] = {}


def _export_containing(address: int, exports_dict: Dict[int, Dict[str,
                                                                 Any]]) -> Optional[int]:
    """
    Return the export `address` falls into, if it sits within a stolen prologue's
    distance of one. Exports only ever live inside mapped modules, so a packer
    allocation can't be mistaken for one at this distance.
    """
    if address in exports_dict:
        return address

    key = len(exports_dict)
    sorted_exports = _SORTED_EXPORTS.get(key)
    if sorted_exports is None:
        sorted_exports = sorted(exports_dict.keys())
        _SORTED_EXPORTS.clear()
        _SORTED_EXPORTS[key] = sorted_exports

    index = bisect.bisect_right(sorted_exports, address) - 1
    if index < 0:
        return None
    candidate = sorted_exports[index]
    if 0 < address - candidate <= STOLEN_PROLOGUE_MAX:
        return candidate
    return None
...
def _unicorn_hook_block(uc: Uc, address: int, _size: int,
    ...
    export_addr = _export_containing(address, exports_dict)
    if export_addr is not None:
        if export_addr != address:
            LOG.debug("Landed at %s, inside export %s (stolen prologue)",
                      hex(address), hex(export_addr))
        address = export_addr
