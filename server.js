...
ipcMain.handle('liveResourceAddItems', async (event, data = {}) => {
  ...
  try {
    ...
    const sourceWeaponsXml = await getLiveSourceWeaponsXml(resourcesRoot, tempRoot, data.serverXbnPath);
    ...
    if(sourceWeaponsXml.outputPath && sourceWeaponsXml.outputPath !== sourceWeaponsXml.path){
...
ipcMain.handle('uiInspectorCommand', async (event, data = {}) => {
  ...
  if(!['show', 'hide', 'toggle', 'reset', 'reload', 'rect', 'scan'].includes(action)){
  ...
  if(!['reset', 'reload', 'scan'].includes(action) && !address){
  ...
  try {
    await fsp.mkdir(carpetaHookUi, { recursive: true });
    let line = 'reset\n';
    if(action === 'scan') line = 'scan\n';
    else if(action === 'reload') line = `reload ${rutaXui}\n`;
