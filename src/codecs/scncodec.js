...
function readXform(r, want){
  ...
  if(r.U8()){
    ...
    let k = r.U32(); for(let j = 0; j < k && !r.bad; j++){ const t = r.U32(); const off = r.o; x.trans.push({ t, off, v: r.V3() }); }
    k = r.U32();     for(let j = 0; j < k && !r.bad; j++){ const t = r.U32(); const off = r.o; x.rot.push({ t, off, q: [r.F32(), r.F32(), r.F32(), r.F32()] }); }
    k = r.U32();     for(let j = 0; j < k && !r.bad; j++){ const t = r.U32(); const off = r.o; x.scale.push({ t, off, v: r.V3() }); }
  }
  return x;
}
...
function readModelAnims(r){
  const anims = []; const c = r.U32();
  for(let i = 0; i < c && !r.bad; i++){
    const a = { name: r.Str(), duration: r.U32() };
    Object.assign(a, readXform(r));
    skipVis(r);
    a.morph = [];
    const mc = r.U32();
...
function applyJson(origBuf, json){
  ...
  for(const [k, node] of byName){
    ...
    wF(buf, node.matrixOff, je.matrix); patched++;
  }
  if(json.clip && json.pose){
    for(const b of s.bones){
      const a = b.anims.find(x => x.name === json.clip && x.hasTransform); const p = json.pose[b.name];
      if(!a || !p) continue;
      if(changed(a.initT, p.initT)) { wF(buf, a.initTOff, p.initT); patched++; }
      if(changed(a.initR, p.initR)) { wF(buf, a.initROff, p.initR); patched++; }
      if(changed(a.initS, p.initS)) { wF(buf, a.initSOff, p.initS); patched++; }
      if(p.rot)   for(let i = 0; i < p.rot.length && i < a.rot.length; i++) if(changed(a.rot[i].q, p.rot[i].q)){ wF(buf, a.rot[i].off, p.rot[i].q); patched++; }
      if(p.trans) for(let i = 0; i < p.trans.length && i < a.trans.length; i++) if(changed(a.trans[i].v, p.trans[i].v)){ wF(buf, a.trans[i].off, p.trans[i].v); patched++; }
      if(p.scale) for(let i = 0; i < p.scale.length && i < a.scale.length; i++) if(changed(a.scale[i].v, p.scale[i].v)){ wF(buf, a.scale[i].off, p.scale[i].v); patched++; }
    }
  }
    ...
    jm2.morph.forEach((ja, ai) => {
      ...
      (ja.keys || []).forEach((jk, ki) => {
        ...
        (jk.verts || []).forEach((jv, vi) => {
          ...
          if(changed(vt.pos, jv.pos)){ wF(buf, vt.off, jv.pos); patched++; }
        });
      });
    });
  });
...
module.exports = { parse, scnToJson, applyJson };
